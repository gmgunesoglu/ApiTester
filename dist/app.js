...
function parsePaths(paths) {
    ...
    for (const keyPath in paths) {
        ...
        const methods = paths[keyPath];
        for (const keyMethod in methods) {
            console.log(`Method: '${keyMethod}'`);
            const requestProperties = methods[keyMethod];
...
async function main() {
    ...
    try {
        ...
        const paths = $refs.get("swagger2.yml#/paths");
        //console.log(paths);
        const schemas = $refs.get("swagger2.yml#/components/schemas/Pet/properties");
        //console.log(schemas);
        parsePaths(paths);
